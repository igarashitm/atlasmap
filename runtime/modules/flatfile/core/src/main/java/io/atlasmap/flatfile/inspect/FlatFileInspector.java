...
import io.atlasmap.flatfile.v2.AtlasFlatFileModelFactory;
import io.atlasmap.flatfile.v2.FlatFileDefinition;
import io.atlasmap.flatfile.v2.FlatFileDocument;
import io.atlasmap.v2.CollectionType;
import io.atlasmap.v2.FieldStatus;
import io.atlasmap.v2.FieldType;

public class FlatFileInspector {

    private static final Logger LOG = LoggerFactory.getLogger(FlatFileInspector.class);
    private static FlatFileInspector myself = new FlatFileInspector();

    private FlatFileInspector() {
    }

    public static FlatFileInspector instance() {
        return myself;
    }

    public FlatFileDocument inspect(String schema) throws FlatFileInspectionException {
        if (schema == null || schema.isEmpty()) {
            throw new IllegalArgumentException("Flat File Definition schema cannot be null");
        }

        try {
            FlatFileDocument jsonDocument = AtlasFlatFileModelFactory.createFlatFileDocument();
            ObjectMapper objectMapper = new ObjectMapper(new YAMLFactory());
            FlatFileDefinition ffd = objectMapper.readValue(schema, FlatFileDefinition.class);
            ...
            throw new FlatFileInspectionException(e);
        }
    }
    ...
    private List<JsonField> loadProperties(JsonNode node, String parentPath, Map<String, JsonNode> definitionMap) throws FlatFileInspectionException {
    ...
    private JsonFieldBuilder getJsonFieldBuilder(String name, JsonNode value, String parentPath, Map<String, JsonNode> definitionMap) throws FlatFileInspectionException {
